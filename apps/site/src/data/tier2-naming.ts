...
export const variantVocab: VocabRow[] = [
  { rubric: 'brand', current: 'brand', status: 'match', note: 'Now an exact match. It was `primary`, which meant two different things at once — the brand hue, and the most prominent of a set (`--color-text-primary`). Property-first naming collapsed those into one slot, so the hue took `brand` and `primary` was left to mean prominence only. `secondary` survived for a while as a brand VARIANT (`--color-background-brand-secondary`), but it was never a second fill: it sat on Radix step 8, the *hovered UI element border* step, and measured 3.51-5.47:1 against its own foreground. On 2026-08-18 it merged into `--color-background-brand-muted` (step 3), which already held the value it should have had. So the brand has ONE fill and one tinted surface, and no variant slot at all.' },
