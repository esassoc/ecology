...
const SCALE_FAMILIES: { family: string; re: RegExp; note: string }[] = [
  ...
  // `--shadow-offset-x` is excluded on purpose: it is a single shared token, not a
  // rung, and `split('-').pop()` would otherwise read its `x` as an escape hatch
  // alongside `--radius-full`. Every remaining member IS on the 050–500 ramp.
  { family: '--shadow-*', re: /^--shadow-(?!offset-x$)/, note: 'Four axes — offset-y, blur, spread and colour — sharing one padded ramp, which is what is left at tier 1 now that the composites moved up to `--elevation-*`. Was the one ragged member (`--shadow-50`, the only sub-100 step in the system written without padding); that was fixed first, and then the family stopped holding composites at all.' },
