...
export function injectStyles(): void {
  ...
  s.textContent = `
    ...
    .esa-field__input, .esa-field__select, .esa-field__textarea {
      ...
      box-shadow:inset 0 0 0 1px var(--_field-border-color);
      border-radius:.25rem; font-family:inherit; font-size:.9375rem;
      color:var(--form-text-color,#171717); background:var(--form-bg,#fff);
      transition:box-shadow .12s; outline:none; box-sizing:border-box;
    }
    .esa-field__input:focus, .esa-field__select:focus, .esa-field__textarea:focus {
      --_field-border-color:var(--form-border-color-focus,var(--color-primary,#005862));
      box-shadow:inset 0 0 0 1px var(--_field-border-color),0 0 0 3px var(--color-primary-subtle,rgba(0,88,98,.08));
    }
    .esa-field__input::placeholder, .esa-field__textarea::placeholder { color:var(--form-placeholder-color,#737373); }
    .esa-field__input--error, .esa-field__select--error, .esa-field__textarea--error { --_field-border-color:var(--form-border-color-error,#ef4444); }
