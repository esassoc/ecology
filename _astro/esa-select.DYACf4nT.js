import{b as o,i as c,A as r,a as h}from"./lit-element.D8DSg5zn.js";import{t as p}from"./typography.KBHeYOQc.js";import{a as u}from"./a11y.sqk3bMt7.js";const f=o`<svg
  class="error__icon"
  viewBox="0 0 24 24"
  fill="none"
  stroke="currentColor"
...
>
  ...
  />
    ...
    </span>`}onTypeahead(e){if(e.key.length!==1||e.ctrlKey||e.metaKey||e.altKey||this.searchable)return;e.preventDefault(),this._typeahead+=e.key.toLowerCase(),clearTimeout(this._typeaheadTimer),this._typeaheadTimer=setTimeout(()=>{this._typeahead=""},500);const i=this.options.findIndex(t=>!t.disabled&&t.label.toLowerCase().startsWith(this._typeahead));i<0||(this._open||this.openDropdown(),this._active=i)}get describedBy(){return[this.errorText?"error":"",this.helpText?"help":"","cue"].filter(Boolean).join(" ")}get cueText(){return this.cue?this.cue:"Use the up and down arrows to review options, or type to jump to one. Enter to choose."}renderTrigger(){const e=this.multiple?this.selectedOptions.map(t=>t.label).join(", "):this.displayValue,i=!e;return o`<button
      type="button"
      ...
      aria-invalid=${this.errorText?"true":r}
      aria-describedby=${this.describedBy||r}
      aria-controls=${this._open?"listbox":r}
      aria-activedescendant=${this._open&&this._active>=0?`opt-${this._active}`:r}
    ...
    >
      ...
      @keydown=${this.onKeydown}
    />`}focus(e){const i=this.renderRoot?.querySelector(".input");i?i.focus(e):super.focus(e)}render(){const e=!!this.errorText;return o`
      <div class="field ${e?"field--error":""}">
        ${this.label?o`<span class="field__label typography-${b[this.size]}" id="label">
              ${this.label}${this.required?o`<span class="field__required" aria-hidden="true">*</span>`:null}
            </span>`:null}
        ...
        <div class="container">
          ...
          ${this._open?o`<div class="dropdown" role="listbox" id="listbox">
                ${this.filteredOptions.length===0?o`<div class="option option--empty typography-${l[this.size]}">No results found</div>`:this.filteredOptions.map((i,t)=>{const a=this.isSelected(i.value);return o`<div
                      ...
                      >
                        ${this.multiple?o`<span class="check ${a?"check--selected":""}">${this.checkIcon()}</span>`:null}
