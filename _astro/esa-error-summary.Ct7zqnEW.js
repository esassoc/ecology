import{i as a,A as l,b as i,a as d}from"./lit-element.D8DSg5zn.js";import{t as c}from"./typography.KBHeYOQc.js";import{a as h}from"./a11y.sqk3bMt7.js";class u extends a{constructor(){super(),this.onLinkClick=(e,r)=>{e.preventDefault();const o=this.getRootNode().getElementById?.(r)??document.getElementById(r);o&&(o.scrollIntoView({block:"center",behavior:"smooth"}),o.focus?.({preventScroll:!0}))},this.errors=[],this.heading="There is a problem",this.headingLevel=2}static{this.properties={errors:{type:Array},heading:{type:String},headingLevel:{type:Number,attribute:"heading-level"}}}get valid(){return(this.errors??[]).filter(e=>e&&String(e.message??"").trim())}focus(e){this.valid.length!==0&&this.updateComplete.then(()=>{this.renderRoot.querySelector(".root")?.focus(e)})}render(){const e=this.valid;if(e.length===0)return l;const r=e.length,t=`${this.heading} — ${r} error${r===1?"":"s"}`,o=Math.min(6,Math.max(2,Number(this.headingLevel)||2));return i`
      <div
        class="root"
        tabindex="-1"
        role="group"
