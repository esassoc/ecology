import{i as u,b as s,a as y}from"./lit-element.C8p3bJxG.js";import{o as v,a as d}from"./unsafe-svg.BgYkCkso.js";const f=a=>a.replace(/[&<>"']/g,e=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[e]),p=(a,e)=>{const t=f(a);if(!e)return t;const r=e.replace(/[.*+?^${}()|[\]\\]/g,"\\$&");return t.replace(new RegExp(`(${r})`,"ig"),"<mark>$1</mark>")};class m extends u{constructor(){super(),this.query="",this.activeScope="",this.activeId=null,this.onGlobalKeydown=e=>{this.hotkey==="mod+k"&&(e.metaKey||e.ctrlKey)&&e.key.toLowerCase()==="k"?(e.preventDefault(),this.toggle()):this.hotkey==="slash"&&e.key==="/"&&!this.isEditable(e.target)&&(e.preventDefault(),this.show())},this.onSearch=e=>{this.query=e.target.value,this.activeId=null},this.onKeydown=e=>{if(e.key==="Escape"){e.preventDefault(),this.close();return}if(e.key==="Tab"){e.preventDefault(),this.cycleScope(e.shiftKey?-1:1);return}if(e.key==="Enter"&&(e.metaKey||e.ctrlKey)){e.preventDefault(),this.emit("show-all",{query:this.query,scope:this.activeScope}),this.close();return}const t=this.flatItems;if(t.length===0)return;const r=t.findIndex(i=>i.id===this.activeId);switch(e.key){case"ArrowDown":{e.preventDefault();const i=r<t.length-1?r+1:0;this.activeId=t[i].id;break}case"ArrowUp":{e.preventDefault();const i=r>0?r-1:t.length-1;this.activeId=t[i].id;break}case"Enter":{e.preventDefault();const i=t.find(o=>o.id===this.activeId)??(t.length===1?t[0]:null);i&&this.selectEntity(i);break}}},this.entities=[],this.scopes=[],this.recent=[],this.rowActions=[],this.open=!1,this.placeholder="Search…",this.allLabel="All",this.hotkey=""}static{this.properties={entities:{type:Array},scopes:{type:Array},recent:{type:Array},rowActions:{type:Array,attribute:"row-actions"},open:{type:Boolean,reflect:!0},placeholder:{type:String},allLabel:{type:String,attribute:"all-label"},hotkey:{type:String},query:{state:!0},activeScope:{state:!0},activeId:{state:!0}}}connectedCallback(){super.connectedCallback(),document.addEventListener("keydown",this.onGlobalKeydown)}disconnectedCallback(){super.disconnectedCallback(),document.removeEventListener("keydown",this.onGlobalKeydown)}isEditable(e){const t=e;if(!t)return!1;const r=t.tagName;return r==="INPUT"||r==="TEXTAREA"||r==="SELECT"||t.isContentEditable}toggle(){this.open?this.close():this.show()}show(){this.open=!0,this.query="",this.activeScope="",this.activeId=null,requestAnimationFrame(()=>{this.renderRoot.querySelector(".esa-entity-search__input")?.focus()})}close(){this.open=!1}emit(e,t){this.dispatchEvent(new CustomEvent(e,{detail:t,bubbles:!0,composed:!0}))}get queryMatches(){const e=this.query.toLowerCase().trim();return e?this.entities.filter(t=>`${t.title} ${t.subtitle??""}`.toLowerCase().includes(e)):this.entities}scopeCount(e){return this.queryMatches.filter(t=>t.scope===e).length}get renderGroups(){const e=t=>this.scopes.find(r=>r.id===t);if(this.activeScope){const t=e(this.activeScope),r=this.queryMatches.filter(i=>i.scope===this.activeScope);return t&&r.length?[{scope:t,items:r}]:[]}return this.scopes.map(t=>({scope:t,items:this.queryMatches.filter(r=>r.scope===t.id)})).filter(t=>t.items.length>0)}get showingRecent(){return!this.query.trim()&&!this.activeScope&&this.recent.length>0}get flatItems(){return this.showingRecent?this.recent:this.renderGroups.flatMap(e=>e.items)}setScope(e){this.activeScope=e,this.activeId=null,this.emit("scope-change",{scope:e})}cycleScope(e){const t=["",...this.scopes.map(o=>o.id)],i=(t.indexOf(this.activeScope)+e+t.length)%t.length;this.setScope(t[i])}selectEntity(e){this.emit("select",{entity:e}),this.close()}onRowAction(e,t,r){e.stopPropagation(),this.emit("row-action",{action:t.id,entity:r})}iconFor(e){return e.icon??this.scopes.find(t=>t.id===e.scope)?.icon}renderIcon(e){return e?s`<svg class="esa-entity-search__icon" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">${v(e)}</svg>`:null}renderRow(e){const t=this.rowActions.filter(r=>!r.scopes||r.scopes.includes(e.scope));return s`
      ...
      >
        ...
        <span class="esa-entity-search__row-text">
          <span class="esa-entity-search__row-title">${d(p(e.title,this.query.trim()))}</span>
          ${e.subtitle?s`<span class="esa-entity-search__row-subtitle">${d(p(e.subtitle,this.query.trim()))}</span>`:null}
        </span>
        ...
        ${t.length?s`<span class="esa-entity-search__row-actions">
              ${t.map(r=>s`<button
                  ...
                  title=${r.label}
                  aria-label=${r.label}
                  @click=${i=>this.onRowAction(i,r,e)}
                >
                  ${r.icon?this.renderIcon(r.icon):s`<span>${r.label}</span>`}
                </button>`)}
            </span>`:null}
      </button>
    `}render(){if(!this.open)return s``;const e=this.query.trim(),t=this.renderGroups,r=this.showingRecent,i=this.queryMatches.length;return s`
      ...
      <div class="esa-entity-search" role="dialog" aria-label="Search">
        ...
        ${this.scopes.length?s`<div class="esa-entity-search__scopes" role="tablist">
              <button
                ...
                @click=${()=>this.setScope("")}
              >
                ${this.allLabel}${e?s`<span class="esa-entity-search__scope-count">${i}</span>`:null}
              </button>
              ${this.scopes.map(o=>s`<button
                ...
                >
                  ${this.renderIcon(o.icon)}${o.label}${e?s`<span class="esa-entity-search__scope-count">${this.scopeCount(o.id)}</span>`:null}
                </button>`)}
            </div>`:null}
