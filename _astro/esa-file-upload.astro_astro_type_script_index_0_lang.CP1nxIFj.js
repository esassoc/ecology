import{i as s,b as o,a as n}from"./lit-element.D8DSg5zn.js";import{t as l}from"./typography.KBHeYOQc.js";import{a as c}from"./a11y.sqk3bMt7.js";import{a}from"./announcer.D25EqVSf.js";class d extends s{constructor(){super(),this.openFileBrowser=()=>{if(this.disabled)return;this.renderRoot.querySelector(".native")?.click()},this.onFileSelected=e=>{const r=e.target;r.files&&this.processFiles(Array.from(r.files)),r.value=""},this.onDragOver=e=>{e.preventDefault(),e.stopPropagation(),this.disabled||(this._isDragging=!0)},this.onDragLeave=e=>{e.preventDefault(),e.stopPropagation(),this._isDragging=!1},this.onDrop=e=>{if(e.preventDefault(),e.stopPropagation(),this._isDragging=!1,this.disabled)return;const r=e.dataTransfer?.files;r&&this.processFiles(Array.from(r))},this.onZoneKeydown=e=>{(e.key==="Enter"||e.key===" ")&&(e.preventDefault(),this.openFileBrowser())},this.label="Upload files",this.accept="",this.multiple=!1,this.maxSizeMb=10,this.disabled=!1,this.name="files",this._isDragging=!1,this._files=[],this._error="",this.internals=this.attachInternals()}static{this.formAssociated=!0}static{this.properties={label:{type:String},accept:{type:String},multiple:{type:Boolean},maxSizeMb:{type:Number,attribute:"max-size-mb"},disabled:{type:Boolean,reflect:!0},name:{type:String,reflect:!0},_isDragging:{state:!0},_files:{state:!0},_error:{state:!0}}}updated(){this.toggleAttribute("dragging",this._isDragging)}syncFormValue(){const e=new FormData;for(const r of this._files)e.append(this.name,r,r.name);this.internals.setFormValue(this._files.length?e:null),this.dispatchEvent(new CustomEvent("change",{detail:{files:[...this._files]},bubbles:!0,composed:!0}))}removeFile(e){const r=this._files[e];this._files=this._files.filter((t,i)=>i!==e),this.syncFormValue(),r&&a(`${r.name} removed. ${this._files.length} remaining.`)}formatFileSize(e){return e<1024?e+" B":e<1024*1024?(e/1024).toFixed(1)+" KB":(e/(1024*1024)).toFixed(1)+" MB"}processFiles(e){this._error="";const r=this.maxSizeMb*1024*1024,t=e.filter(i=>i.size>r);t.length>0&&(this._error=`File${t.length>1?"s":""} exceed ${this.maxSizeMb} MB limit: ${t.map(i=>i.name).join(", ")}`,a(this._error,{assertive:!0}),e=e.filter(i=>i.size<=r)),e.length!==0&&(this._files=this.multiple?[...this._files,...e]:[e[0]],this.syncFormValue(),a(`${e.length} file${e.length>1?"s":""} added. ${this._files.length} total.`))}render(){return o`
      <input
        type="file"
        class="native"
        accept=${this.accept}
      ...
      >
        ...
        ${this.maxSizeMb?o`<span class="zone__limit typography-body-sm">Max ${this.maxSizeMb} MB per file</span>`:null}
      </div>

      ${this._error?o`<div class="error typography-body-sm">${this._error}</div>`:null}

      ${this._files.length>0?o`<ul class="files">
            ${this._files.map((e,r)=>o`<li class="file">
                ${this.fileIcon()}
                ...
                <button
                  ...
                  @click=${t=>{t.stopPropagation(),this.removeFile(r)}}
                >
                  ${this.xIcon()}
                </button>
              </li>`)}
          </ul>`:null}
    `}uploadIcon(){return o`<svg class="icon icon--lg" viewBox="0 0 24 24" fill="none" stroke="currentColor"
      stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">
      <path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4" /><polyline points="17 8 12 3 7 8" />
      <line x1="12" y1="3" x2="12" y2="15" /></svg>`}fileIcon(){return o`<svg class="icon icon--sm" viewBox="0 0 24 24" fill="none" stroke="currentColor"
      stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">
      <path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z" /><polyline points="14 2 14 8 20 8" /></svg>`}xIcon(){return o`<svg class="icon icon--sm" viewBox="0 0 24 24" fill="none" stroke="currentColor"
      stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">
