import{w as n,i as c,b as t,a as l}from"./lit-element.C8p3bJxG.js";const h=n`<polyline points="20 6 9 17 4 12"></polyline>`;class d extends c{constructor(){super(),this.toggleOption=e=>{if(e.disabled)return;const o=this.value.indexOf(e.value);this.value=o>=0?this.value.filter(s=>s!==e.value):[...this.value,e.value],this.syncFormValue(),this.dispatchEvent(new CustomEvent("change",{detail:{value:this.value},bubbles:!0,composed:!0}))},this.onKeydown=(e,o)=>{(e.key===" "||e.key==="Enter")&&(e.preventDefault(),this.toggleOption(o))},this.options=[],this.label="",this.size="md",this.orientation="vertical",this.name="",this.value=[],this.internals=this.attachInternals()}static{this.formAssociated=!0}static{this.properties={options:{type:Array},label:{type:String},size:{type:String,reflect:!0},orientation:{type:String,reflect:!0},name:{type:String},value:{type:Array}}}willUpdate(e){if(e.has("options")&&typeof this.options=="string")try{this.options=JSON.parse(this.options)}catch{this.options=[]}}connectedCallback(){super.connectedCallback(),this.syncFormValue()}syncFormValue(){const e=new FormData,o=this.name||"checkbox-group";for(const s of this.value)e.append(o,s);this.internals.setFormValue(e)}isChecked(e){return this.value.includes(e)}render(){return t`
      ${this.label?t`<span class="group-label">${this.label}</span>`:null}
      <div class="items" role="group" aria-label=${this.label}>
        ${this.options.map(e=>{const o=this.isChecked(e.value),s=e.disabled??!1;return t`
            <label
            ...
            >
              ...
              >
                ${o?t`<svg class="icon" viewBox="0 0 24 24" fill="none" stroke="currentColor"
