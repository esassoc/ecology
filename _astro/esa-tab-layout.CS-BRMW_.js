import{i,b as r,a as n}from"./lit-element.D8DSg5zn.js";import{t as l}from"./typography.KBHeYOQc.js";const c={xs:"microcopy-xs-subtle",sm:"microcopy-sm-subtle",md:"microcopy-md-subtle",lg:"microcopy-lg-subtle"},d={xs:"microcopy-xs",sm:"microcopy-sm",md:"microcopy-md",lg:"microcopy-lg"};class b extends i{constructor(){super(),this.onKeydown=(t,e)=>{let a=null;switch(t.key){case"ArrowRight":a=this.findNextEnabledTab(e,1);break;case"ArrowLeft":a=this.findNextEnabledTab(e,-1);break;case"Home":a=this.findNextEnabledTab(-1,1);break;case"End":a=this.findNextEnabledTab(this.tabs.length,-1);break;default:return}a!==null&&(t.preventDefault(),this.selectTab(a),t.target.parentElement?.children[a]?.focus())},this.tabs=[],this.activeIndex=0,this.size="md",this.variant="underline",this.appearance="underline"}static{this.properties={tabs:{type:Array},activeIndex:{type:Number,attribute:"active-index"},size:{type:String,reflect:!0},variant:{type:String,reflect:!0},appearance:{type:String,reflect:!0}}}selectTab(t){this.tabs[t]?.disabled||(this.activeIndex=t,this.dispatchEvent(new CustomEvent("tabchange",{detail:{index:t},bubbles:!0,composed:!0})))}findNextEnabledTab(t,e){let a=t+e;for(;a>=0&&a<this.tabs.length;){if(!this.tabs[a].disabled)return a;a+=e}return null}render(){return r`
      <div class="layout">
        <div class="tabs" part="tabs" role="tablist">
          ${this.tabs.map((t,e)=>{const a=this.activeIndex===e,o=a?d[this.size]:c[this.size];return r`<button
              class="tab typography-${o} ${a?"tab--active":""} ${t.disabled?"tab--disabled":""}"
              type="button"
              role="tab"
              aria-selected=${a}
              tabindex=${a?0:-1}
            ...
            >
              ${t.icon?r`<span class="icon" .innerHTML=${t.icon}></span>`:null}
              <span>${t.label}</span>
              ${t.badge!=null?r`<span class="badge typography-microcopy-xs-strong">${t.badge}</span>`:null}
            </button>`})}
        </div>
