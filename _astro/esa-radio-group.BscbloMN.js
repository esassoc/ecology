import{b as t,i as n,A as r,a as d}from"./lit-element.D8DSg5zn.js";import{t as c}from"./typography.KBHeYOQc.js";import{a as h}from"./a11y.sqk3bMt7.js";const p=t`<svg
  class="error__icon"
  viewBox="0 0 24 24"
  fill="none"
  stroke="currentColor"
...
>
  <circle cx="12" cy="12" r="10" /><line x1="12" x2="12" y1="8" y2="12" /><line
    ...
    y2="16"
  />
</svg>`,u={xs:"label-2xs",sm:"label-xs",md:"label-md",lg:"label-lg"},b={xs:"body-2xs",sm:"body-xs",md:"body-md",lg:"body-lg"};class g extends n{constructor(){super(),this.selectOption=e=>{e.disabled||(this.value=e.value,this.internals.setFormValue(this.value),this.dispatchEvent(new CustomEvent("change",{detail:{value:this.value},bubbles:!0,composed:!0})))},this.onKeydown=(e,i)=>{(e.key===" "||e.key==="Enter")&&(e.preventDefault(),this.selectOption(i))},this.options=[],this.label="",this.size="md",this.orientation="vertical",this.value=null,this.required=!1,this.helpText="",this.errorText="",this.liveError=!1,this.internals=this.attachInternals()}static{this.formAssociated=!0}static{this.properties={options:{type:Array},label:{type:String},size:{type:String,reflect:!0},orientation:{type:String,reflect:!0},name:{type:String,reflect:!0},value:{type:String},required:{type:Boolean},helpText:{type:String,attribute:"help-text"},errorText:{type:String,attribute:"error-text"},liveError:{type:Boolean,attribute:"live-error"}}}updated(){this.syncValidity()}syncValidity(){if(!this.required||this.value){this.internals.setValidity({});return}const e=this.renderRoot?.querySelector(".circle")??void 0;this.internals.setValidity({valueMissing:!0},this.label?`Select ${this.label}.`:"Select an option.",e)}willUpdate(e){if(e.has("options")&&typeof this.options=="string")try{this.options=JSON.parse(this.options)}catch{this.options=[]}e.has("value")&&this.internals.setFormValue(this.value)}connectedCallback(){super.connectedCallback(),this.internals.setFormValue(this.value)}isSelected(e){return this.value===e}focus(e){const i=this.renderRoot?.querySelector(".circle--selected, .circle");i?i.focus(e):super.focus(e)}render(){const e=!!this.errorText,i=[e?"error":"",this.helpText?"help":""].filter(Boolean).join(" ");return t`
      ...
      >
        ${this.label?t`<legend class="group-label typography-${u[this.size]}" id="legend">
              ${this.label}${this.required?t`<span class="required" aria-hidden="true">*</span>`:null}
            </legend>`:null}
        ${this.options.map(o=>{const a=this.isSelected(o.value),s=o.disabled??!1;return t`
            <label
              class="item ${s?"item--disabled":""}"
              @keydown=${l=>this.onKeydown(l,o)}
              @click=${()=>this.selectOption(o)}
            >
              <span
                class="circle ${a?"circle--selected":""}"
                role="radio"
              ...
              <span class="item-label typography-${b[this.size]}">${o.label}</span>
            </label>
          `})}
      </fieldset>
      ...
      >${e?t`${p}<span class="visually-hidden">Error: </span
  ...
  `]}}customElements.get("esa-radio-group")||customElements.define("esa-radio-group",g);
