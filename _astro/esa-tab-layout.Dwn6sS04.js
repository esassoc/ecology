import{i as o,b as r,a as n}from"./lit-element.D8DSg5zn.js";class s extends o{constructor(){super(),this.onKeydown=(a,e)=>{let t=null;switch(a.key){case"ArrowRight":t=this.findNextEnabledTab(e,1);break;case"ArrowLeft":t=this.findNextEnabledTab(e,-1);break;case"Home":t=this.findNextEnabledTab(-1,1);break;case"End":t=this.findNextEnabledTab(this.tabs.length,-1);break;default:return}t!==null&&(a.preventDefault(),this.selectTab(t),a.target.parentElement?.children[t]?.focus())},this.tabs=[],this.activeIndex=0,this.size="md",this.variant="underline",this.appearance="underline"}static{this.properties={tabs:{type:Array},activeIndex:{type:Number,attribute:"active-index"},size:{type:String,reflect:!0},variant:{type:String,reflect:!0},appearance:{type:String,reflect:!0}}}selectTab(a){this.tabs[a]?.disabled||(this.activeIndex=a,this.dispatchEvent(new CustomEvent("tabchange",{detail:{index:a},bubbles:!0,composed:!0})))}findNextEnabledTab(a,e){let t=a+e;for(;t>=0&&t<this.tabs.length;){if(!this.tabs[t].disabled)return t;t+=e}return null}render(){return r`
    ...
    :host {
      ...
      --_tab-badge-color: var(--color-content-inverse, #ffffff);
  ...
  `}}customElements.get("esa-tab-layout")||customElements.define("esa-tab-layout",s);
