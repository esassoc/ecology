// ─────────────────────────────────────────────────────────────────────────────
// TEMPORARY VENDORED COPY — DELETE WHEN `component-refinement` LANDS ON main.
//
// The original is apps/site/src/data/component-api.ts, which exists ONLY on the
// component-refinement branch (added in db7bd7b). The Storybook branch is
// deliberately rebased onto main so it can be merged or dropped on its own, and
// on main that file is not there — the story generator failed to resolve it.
//
// This copy is byte-identical to component-refinement's EXCEPT for ROOT below,
// which was `'..','..','..','..'` from apps/site/src/data and is `'..','..'`
// from scripts/lib.
//
// WHEN component-refinement MERGES: delete this file and repoint the import in
// scripts/generate-stories.ts back at apps/site/src/data/component-api.ts.
// Two copies of a source-of-truth extractor is exactly the drift this repo
// exists to prevent — it is tolerable only because it is temporary and the
// duplication is announced here.
// ─────────────────────────────────────────────────────────────────────────────
