...
- Flag any remaining **hardcoded** color/spacing/radius values in shared components for tokenization.
...
- **Slice 0b — App shell / IA (when the spoke restructures the frame):** if the spoke's `AppShell` differs from the app's current shell (e.g. spoke = collapsible **sidebar**, app = **header nav**), restructure the frame to match *before* the component slices — everything renders inside it. This is a **design-led structural change** handled by `ecology-migrate-shell` (not a per-component swap); use the spoke's `AppShell` + any Beacon `modern-layout` reference it ports. *(Detect: the spoke ships `src/layouts/AppShell*` with a sidebar / collapsible rail — `.side-nav` / `modern-layout` BEM — while the app's current shell is a top header nav.)*
- **Slice 1 — Leaf DIRECT primitives:** stateless presentational swaps (button, icon, badge, breadcrumb, alert-box, page-header, stat, icon-button, pagination). No form/state wiring → lowest risk; proves the token + web-component plumbing.
