...
**Token tiers** — primitive (`--color-*-NNN`, `--spacing-NNN`, `--radius-NNN`, type/effect) → semantic (`--color-background-brand`/`-hover`/`-active`, `--color-background-brand-secondary`, `--color-background-elevation-raised`/`-elevated`/`-sunken`, `--color-background-default`, `--color-border-default`, `--color-text-*`, status `--color-background-info`/`-success`/`-warning`/`-danger`) → component (`--form-*`, `--card-*`, `--sidenav-*`, `--topbar-*`, `--breadcrumbs-*`, `--grid-*`, `--filter-*`, `--alert-box-*`, `--dialog-*`, `--side-dialog-*`, `--badge-*`, `--button-*`).
