...
- Map the app's existing CSS custom properties → Ecology **semantic** tokens (e.g. `--primary` → `--color-background-brand`, raw `--gray-200` used as a border → `--color-border`).
...
**Token tiers** — primitive (`--color-*-NNN`, `--spacing-NNN`, `--radius-NNN`, type/effect) → semantic (`--color-background-brand`/`-hover`/`-active`, `--color-background-brand-secondary`, `--color-background-raised`/`-elevated`/`-sunken`, `--color-background`, `--color-border`, `--color-text-*`, status `--color-background-info`/`-success`/`-warning`/`-danger`) → component (`--form-*`, `--card-*`, `--sidenav-*`, `--topbar-*`, `--breadcrumbs-*`, `--grid-*`, `--filter-*`, `--alert-box-*`, `--dialog-*`, `--side-dialog-*`, `--badge-*`, `--button-*`).
