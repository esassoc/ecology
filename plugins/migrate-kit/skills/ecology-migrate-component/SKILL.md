...
- **Icons: input for leading, projection for the rest.** A lego with an icon input takes one *leading* glyph via the input; *trailing* / both-side / extra glyphs are projected into content. Map **every** legacy glyph source — an icon font (`<i class="…">`), the app's own icon component, or a literal text char in the label — to the lego's icon system, and **keep the app's icon component imported** if a non-migrated glyph remains elsewhere in the template.
