...
**Migrating native-element consumers — rules learned in the field:**
...
- **Import-only edits can trip a repo guardrail on pre-existing code.** Adding just an `imports` entry to a component can fire an rxjs/lint PreToolUse/PostToolUse hook over *unrelated, pre-existing* code in that file — `.subscribe()`/side-effects, or even plain DTO/context-container classes with public array fields (a frequent false positive). That's not a regression from this migration — don't "fix" out-of-scope code to satisfy it; note it and move on.
- **Trailing / both-side icons.** The button lego's `icon=` input is *leading-only*. For a text button with a *trailing* glyph ("Next →"), project the `<esa-icon>` into the button's content after the label; for both sides, leading via `icon=` + trailing in content. (Add a `trailingIcon` input to the lego if it recurs.)
- **Two legacy icon sources, one target.** A button's glyph may be FontAwesome (`<i class="fa…">`) *or the app's own icon component* (`<icon icon="CircleX">`, PascalCase names) — both map to a Lucide name in the registry (`CircleX`→`circle-x`, `NavArrowRight`→`arrow-right`, …). When a **non-button** `<icon>` remains in the same template (a header/title glyph), **keep the app's icon component in imports** — don't drop it just because the buttons migrated.
- **`.btn` alongside other classes.** Drop only the `.btn`/`.btn-*` tokens; **preserve any meaningful layout/utility class** on the same element (`class="btn btn-primary w-full"` → keep `w-full`).
