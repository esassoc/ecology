...
- **The Playwright MCP tool names are environment-specific.** This skill's `allowed-tools` names them `mcp__playwright__*`, which assumes a bare MCP server registered as `playwright`. If the app instead adds Playwright as a *plugin*, the tools are namespaced (e.g. `mcp__plugin_playwright_playwright__browser_*`) and those allowlist entries won't match — reconcile the names to your setup. The **scripted CLI Playwright** (the preferred path in Phase 3) needs no MCP allowlist at all, so it sidesteps this entirely; the MCP browser is the fallback.
