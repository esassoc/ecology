...
Follow the handoff's **top-bar + side-nav `decisions`/`gotchas` verbatim** (e.g. the collapse toggle, the 280↔72px rail, the topbar grid, the stacking/z-index rules). **Compose the chrome leaf-legos** (`esa-icon`/`-icon-button`/`-badge`) — these should be built **before** the shell (audit **Slice 0a**) precisely so the shell composes reusable legos rather than inlining its own. **If they aren't built yet,** inline the prototype's own glyphs/buttons in the new structure (the spoke `AppShell`'s inline Lucide svgs) and **flag them for retro-adoption** when the legos land — don't block, but don't bake a *duplicate* lego either (the same icon-button markup/CSS pasted into the shell, the grid toolbar, and the next screen is the anti-pattern). Prefer the conductor sequence Slice 0a first. *(The first dogfood shipped the shell inline + retrofit; front-loading the legos avoids that churn.)* Read tokens; for a rung the app's theme lacks (e.g. a brand-ramp shade, a `--spacing-*` step, or a `--font-weight-*` the theme doesn't define), either **add the missing rung to the theme bridge** (preferred when it's a real scale gap — coordinate with the apply-theme step) or rely on the spoke's literal fallback, and **say which**.
...
- **The shell owns the root + composes the topbar icon-buttons — two token-cleanliness hooks live here.** (1) Bind the global `body`/root text color to `var(--color-text-primary)` if the app leaves it uncolored (so it inherits the browser-default `#000`) — the highest-leverage consumption fix, so all *inherited* text follows a theme override (~a no-op in light; see `ecology-audit`). (2) When a topbar `esa-icon-button`'s label is **dynamic**, pass it through the lego's aliased **input** `[aria-label]="…"`, **not** `[attr.aria-label]` — the lego host-binds the attr from its own input and silently swallows a raw attr binding (see `ecology-migrate-component`). If the shell adds a theme toggle, apply the saved theme in the **root component constructor** — before the shell paints *and* before any lazy grid/map module resolves its JS theme at load.
