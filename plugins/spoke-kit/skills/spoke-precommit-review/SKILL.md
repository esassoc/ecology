...
1. **Ecology token usage** — map raw hex/px to semantic tokens (`#1e5386`→`--color-background-brand`, `#f3f7fc`→`--color-background-brand-subtle`, IBM Plex→`--typography-font-family-sans`/`--typography-font-family-display`, spacing→`--spacing-*`, radius→`--radius-100`/`--radius-200`). **Verify every referenced custom property actually exists** by grepping `node_modules/@esa/tokens/`. Recurring chrome values (2+ uses) → promote to `theme-<brand>.css` as `--cbf-*` or a reassigned semantic token. Primitives never move — re-point the consuming semantic token. `--cbf-*` raw-ramp tokens live ONLY in the theme file; components never read them directly.
2. **Type contract** — when a component overrides `--typography-font-family-sans`/`--typography-font-family-display`, every `font-weight` it uses must be in the Google Fonts `<link>` request in `BaseLayout.astro`. (Real bug: `font-weight: 500` used, link requested only `400;600` → silent faux-bold.)
