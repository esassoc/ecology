...
2. **Hub gate** — this spoke's `@esa/*` packages are `file:` symlinks into the
   sibling `../ecology` checkout: **whatever state that checkout is in gets
   baked into the deploy.** Check it:
   - `git -C ../ecology fetch`, then read the branch
     (`git -C ../ecology rev-parse --abbrev-ref HEAD`) and working-tree state
     (`git -C ../ecology status --porcelain`).
   - **HARD GATE: if the hub is dirty or not on `main`, the ship stops.**
     Explain in plain language: deploying now would publish unmerged ecology
     work on the public site — park the hub on a clean, pulled `main` (or
     merge the hub work) and re-run /ship. The ONLY way past is the human
     explicitly approving in this conversation; then record it by prefixing
     the step-3 commit message with `ship-wip-approved: <who approved / why>`
     and name the hub branch/state in the final report.
   - If the hub is clean-on-main but behind origin/main:
     `git -C ../ecology pull --ff-only`.
   - Rebuild the standard from that state:
     `npm --prefix ../ecology run build:tokens`. If it fails, the ship stops
     (same rule as the build gate).
3. **Save** — commit all current work with a clear message; push to the main
   branch. (If there's nothing to commit, say so and continue.)
4. **Build** — `npm run build`. **HARD GATE: if the build fails, the ship
   stops.** Explain the failure in one sentence; fix it if unambiguous and
   rebuild, otherwise report and stop.
5. **Quick check (warn, don't block)** — if /design-qa hasn't been run since
...
6. **Deploy** — `npm run deploy`. Known failure: a stale gh-pages cache after
   someone else deployed (`fatal: ...gh-pages` git errors) → run
   `npx gh-pages-clean`, then retry the deploy ONCE.
7. **Report** — give the public URL (read `site` + the production `base` from
